...
# 7. Library (signed in)
# --------------------------------------------------------------------------- #
class LibraryView(TableFitMixin, Widget):
    """The signed-in user's library playlists and liked songs."""
    ...
    def compose(self) -> ComposeResult:
        with Vertical(classes="view_content_container"):
            yield SectionHeader(id="library_header", classes="section_header")
            with TabbedContent(initial="tab_library_playlists", id="library_tabs"):
                with TabPane("playlists", id="tab_library_playlists"):
                    yield DataTable(id="library_playlists_table", cursor_type="row")
                with TabPane("liked", id="tab_library_liked"):
                    yield DataTable(id="library_liked_table", cursor_type="row")

    def on_mount(self) -> None:
        self.query_one("#library_playlists_table", DataTable).add_columns(
            ("#", "num"), ("PLAYLIST", "title"), ("TRACKS", "meta")
        )
        self.query_one("#library_liked_table", DataTable).add_columns(
            ("#", "num"), ("TITLE", "title"), ("ARTIST", "artist"), ("TIME", "time")
        )
        self.set_signed_out()
        self._fit_tables()
    ...
    def _fitted_tables(self):
        try:
            return [
                (self.query_one("#library_playlists_table", DataTable), "playlist"),
                (self.query_one("#library_liked_table", DataTable), "track"),
            ]
        except Exception:
            return []
    ...
    def primary_widget(self) -> Optional[Widget]:
        try:
            tabs = self.query_one("#library_tabs", TabbedContent)
            if tabs.active == "tab_library_liked":
                return self.query_one("#library_liked_table", DataTable)
            return self.query_one("#library_playlists_table", DataTable)
        except Exception:
            return None
    ...
    def set_signed_out(self) -> None:
        ...
        self.playlists = []
        self.liked = []
        self.query_one("#library_header", SectionHeader).set_content(
            "library", [("press g to sign in", False)]
        )
        self.query_one("#library_playlists_table", DataTable).clear()
        self.query_one("#library_liked_table", DataTable).clear()

    def populate(self, playlists: List[Playlist], liked: List[Track]) -> None:
        ...
        self.query_one("#library_header", SectionHeader).set_content(
            "library",
            [
                (_count(len(playlists), "playlist"), True),
                (_count(len(liked), "liked song"), False),
            ],
        )

        pl_table = self.query_one("#library_playlists_table", DataTable)
        pl_table.clear()
        for idx, p in enumerate(playlists, 1):
            pl_table.add_row(str(idx), p.title, p.track_count or "—",
                             key=f"libpl_{p.id}_{idx}")

        liked_table = self.query_one("#library_liked_table", DataTable)
        liked_table.clear()
        for idx, tr in enumerate(liked, 1):
            liked_table.add_row(str(idx), tr.title, tr.artist, tr.display_duration,
                                key=f"lib_{tr.id}_{idx}")
    ...
    def get_selected_track(self) -> Optional[Track]:
        try:
            ...
            if tabs.active != "tab_library_liked":
                return None
            table = self.query_one("#library_liked_table", DataTable)
            if 0 <= table.cursor_row < len(self.liked):
                return self.liked[table.cursor_row]
        except Exception:
            pass
        return None
    ...
    def get_selected_playlist(self) -> Optional[Playlist]:
        try:
            tabs = self.query_one("#library_tabs", TabbedContent)
            if tabs.active != "tab_library_playlists":
                return None
            table = self.query_one("#library_playlists_table", DataTable)
            if 0 <= table.cursor_row < len(self.playlists):
                return self.playlists[table.cursor_row]
        except Exception:
            pass
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        if event.data_table.id == "library_liked_table":
            if 0 <= event.cursor_row < len(self.liked):
                tr = self.liked[event.cursor_row]
                self.post_message(PlayTrackMsg(tr, self.liked[event.cursor_row + 1:]))
        elif event.data_table.id == "library_playlists_table":
            if 0 <= event.cursor_row < len(self.playlists):
                self.post_message(OpenPlaylistMsg(self.playlists[event.cursor_row]))

    def refresh_theme(self) -> None:
        self.query_one("#library_header", SectionHeader).refresh()
